...
export default defineConfig({
  ...
  // `/elk-layout`, `/rust-layout/wasm`) is its own tree-shaking boundary. `splitting` dedupes shared
  // chunks, so the viz-layout wasm (base64) only lands in the `/rust-layout/wasm` chunk, never the
  // core entry.
  entry: [
    "src/index.ts",
    "src/ui/index.ts",
